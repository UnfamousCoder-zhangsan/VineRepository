...
@property (nonatomic, strong) UIView       *adView;       /**< APP广告 */
@end
...
- (instancetype)initWithFrame:(CGRect)frame aDduration:(NSInteger)duration adView:(UIView *)adView hideSkipButton:(BOOL)hideSkip launchAdClickBlock:(DDLaunchAdClickBlock)aDClickBlock {
    self = [super initWithFrame:frame];
    if (self) {
        self.adFrame = frame;
        self.aDduration = duration;
...
- (void)setUpAdView {
    ...
    self.adView.frame = self.adFrame;
    self.adView.userInteractionEnabled = YES;
    self.adView.alpha = 0.2;
