...
+ (instancetype)showDatePickerWithTitle:(NSString *)title
                               dateType:(BRDatePickerMode)dateType
                        defaultSelValue:(NSString *)defaultSelValue
                            resultBlock:(BRDateResultBlock)resultBlock
{
    return [self showDatePickerWithTitle:title dateType:dateType defaultSelValue:defaultSelValue minDate:nil maxDate:nil isAutoSelect:NO themeColor:nil resultBlock:resultBlock cancelBlock:nil];
}
...
+ (instancetype)showDatePickerWithTitle:(NSString *)title
                        ...
                        defaultSelValue:(NSString *)defaultSelValue
                           ...
                           isAutoSelect:(BOOL)isAutoSelect
                            ...
                            resultBlock:(BRDateResultBlock)resultBlock
{
    return [self showDatePickerWithTitle:title dateType:dateType defaultSelValue:defaultSelValue minDate:minDate maxDate:maxDate isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:nil];
}
                  ...
                  cancelBlock:(BRDateCancelBlock)cancelBlock
{
    if (self = [super init]) {
        ...
        if (minDate) {
            ...
            }else if (self.showType == BRDatePickerModeM) {
                self.minLimitDate = [NSDate br_setMonth:1 day:1];
            } else {
        ...
        if (maxDate) {
            ...
            if (self.showType == BRDatePickerModeTime || self.showType == BRDatePickerModeCountDownTimer || self.showType == BRDatePickerModeHM) {
               ...
               self.maxLimitDate = [NSDate br_setMonth:12 day:1];
           } else {
...
- (void)setupSelectDateFormatter:(BRDatePickerMode)model
{
    switch (model) {
        ...
        case BRDatePickerModeM: {
           self.selectDateFormatter = @"MM";
           self.style = BRDatePickerStyleCustom;
       } break;
...
- (void)scrollToSelectDate:(NSDate *)selectDate animated:(BOOL)animated
{
    ...
    } else if (self.showType == BRDatePickerModeYMDP) {
        indexArr = @[@(yearIndex), @(monthIndex), @(dayIndex)];
    } else if (self.showType == BRDatePickerModeYM) {
        indexArr = @[@(yearIndex), @(monthIndex)];
    } else if (self.showType == BRDatePickerModeYMP) {
        indexArr = @[@(yearIndex), @(monthIndex)];
...
- (NSDate *)getDidSelectedDate:(NSInteger)component row:(NSInteger)row
{
    ...
    if (self.showType == BRDatePickerModeYMDHM) {
        ...
        selectDateValue = [NSString stringWithFormat:@"%@", self.monthArr[_monthIndex]];
    } else if (self.showType == BRDatePickerModeMD) {
        if (component == 0) {
            _monthIndex = row;
            [self updateDateArray];
